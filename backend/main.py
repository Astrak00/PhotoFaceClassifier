...
# Determine data directory - supports override via environment variable
def get_data_dir() -> Path:
    """Get the data directory, supporting environment variable override."""
    env_data_dir = os.environ.get("FACE_CLASSIFIER_DATA_DIR")
    if env_data_dir:
        return Path(env_data_dir)

    # For PyInstaller bundled app, use a writable location
...
# Ensure directories exist at module load time (needed for StaticFiles mount)
...
async def lifespan(app: FastAPI):
    ...
    # Initialize face processor with lazy loading (models load on first scan)
    # This makes startup much faster (~2s instead of ~60s)
    print("Initializing face processor (models will load on first use)...")
    face_processor = FaceProcessor(cache_dir=CACHE_DIR, lazy_load=True)

    # Apply configured device (will be used when models load)
    if app_config["device"] != "auto":
        face_processor.requested_device = app_config["device"]

    print("Server ready! Models will load on first scan.")
    ...
    try:
        ...
        if not image_files:
            ...
            db.update_scan_session(
                scan_id, status="completed", completed_at=datetime.utcnow()
            )
            return
        ...
        if all_embeddings:
            ...
            labels, cluster_info = cluster_faces(
                embeddings_array,
                min_photos_per_person=app_config["min_faces_per_person"],
            )
            ...
            try:
                ...
                for cluster_id in unique_labels:
                    ...
                    existing_person = (
                        ...
                        .filter_by(cluster_id=int(cluster_id))
                        .first()
                    )
                    if existing_person:
...
async def merge_persons(request: MergePersonsRequest):
    ...
    try:
        ...
        if not keep:
            raise HTTPException(
                status_code=404, detail=f"Person {request.keep_person_id} not found"
            )
        ...
        for merge_id in request.merge_person_ids:
            ...
            if not merge:
                raise HTTPException(
                    status_code=404, detail=f"Person {merge_id} not found"
                )
...
async def create_export_folders(request: CreateFoldersRequest):
    ...
    if not output_dir.exists():
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Cannot create output directory: {e}"
            )
    ...
    for folder_spec in request.folders:
        ...
        photos = db.get_photos_for_persons(
            folder_spec.person_ids, logic=folder_spec.logic
        )
...
async def get_config():
    ...
    current_device = (
        face_processor.get_device_info()
        if face_processor
        else {"device": "unknown", "device_name": "Unknown"}
    )
...
async def update_config(config: dict):
    ...
    if "min_faces_per_person" in config:
        ...
        if not isinstance(val, int) or val < 1 or val > 20:
            raise HTTPException(
                status_code=400, detail="min_faces_per_person must be between 1 and 20"
            )
    ...
    if "device" in config:
        ...
        if device not in available:
            raise HTTPException(
                status_code=400, detail=f"Invalid device. Available: {available}"
            )
...
def main():
    """Main entry point for the application (used by PyInstaller)."""
    import uvicorn
    import argparse

    parser = argparse.ArgumentParser(description="Face Classifier Backend Server")
    parser.add_argument("--host", default="127.0.0.1", help="Host to bind to")
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("FACE_CLASSIFIER_PORT", "8000")),
    ...
    print(f"Starting Face Classifier Backend...")
